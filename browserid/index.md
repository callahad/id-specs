...
The BrowserID public key format is based on an early [JSON Web Key (JWK)](http://tools.ietf.org/html/draft-ietf-jose-json-web-key-01) draft.

Public keys are represented as JSON objects.
Every public key MUST have an `"algorithm"` property which indicates the type of public key described by the object.
Objects carry additional properties as necessary for each algorithm.
The supported algorithms and their additional properties are described in Appendix A.

For example, a public key may look like this:
